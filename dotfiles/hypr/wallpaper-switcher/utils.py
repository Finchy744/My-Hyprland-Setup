...
def set_current_wallpaper(wallpaper_path: Path):
    ...
    sync_waypaper_config(wallpaper_path)


def sync_waypaper_config(wallpaper_path: Path):
    """Update Waypaper's own config so 'waypaper --restore' at boot picks up the right wallpaper."""
    config_path = Path.home() / ".config/waypaper/config.ini"
